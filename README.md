...
ConnectionPersistence is a Windows-only Rust tray application planned to keep a selected Windows built-in VPN alive whenever a real physical Ethernet link is present.

The app will run primarily from the system tray, reconnect the configured VPN aggressively when it drops, verify that a configurable route target uses the VPN interface, avoid disrupting fullscreen games, and write verbose CSV logs.
...
ConnectionPersistence exists to keep trying. If the selected VPN is down or connected but not routing the configured test target through the VPN interface, the app treats it as unhealthy and keeps reconnecting while a real Ethernet link is present. There is no retry limit, and every attempt is logged.
...
7. Show a safe upper-right test alert or fall back to raw Win32.
8. Write one CSV event row.
